...
// закрыт). Поэтому перед каждой доставкой — короткая проба обоих
// путей: доступность GitHub и скорость каналов меняются, и прошлый итог
// ничего не гарантирует. Последний итог записывается на хост
// (hosts.binary_via) только для сведения; провал выбранного пути
...
// binarySource — что заливать: локальный файл, его копия на GitHub
// (если есть) и куда записать способ, которым доставили.
