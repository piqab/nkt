...
// isSensitiveFile reports whether a path points to a file that should never
// be directly readable through the config API, regardless of whether it lies
// within a managed service root. This check is applied to direct reads,
// version content retrieval, and diffs to prevent authenticated viewers from
// bypassing the directory-listing exclusion by requesting a known path.
func isSensitiveFile(path string) bool {
	name := gopath.Base(path)
	return privateKeyRe.MatchString(name)
}
