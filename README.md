...
* **резервный канал связи на случай недоступности SSH** — включён по
  умолчанию для новых хостов: тот держит независимое от SSH постоянное
  соединение к хабу (WebSocket + TLS), так что дашборд, терминал и
  «обновить»/«переустановить» продолжают работать, даже если входящий
  22-й порт на хосте заблокируют файрволом. Хаб всегда сначала пробует
  обычный SSH, переключается на этот канал только при неудаче дозвона —
  кроме самой первой установки хоста, которая всегда требует настоящего
  SSH. Таблица хостов показывает бейджем, подключён ли канал и используется
  ли он прямо сейчас. Адрес хаба для обратного звонка определяется сам (по
  адресу, которым воспользовался браузер при установке/обновлении) —
  `NKT_HUB_PUBLIC_ADDR` нужен только если это автоопределение угадывает
  неверно (NAT, reverse proxy) — см. HUB.md, раздел 5;
