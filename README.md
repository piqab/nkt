...
curl -fsSLO https://github.com/piqab/nkt/releases/download/v1.8.68/nkt-linux-amd64
curl -fsSLO https://github.com/piqab/nkt/releases/download/v1.8.68/SHA256SUMS
...
| The terminal prints `nsenter: reassociate to namespaces failed: Operation not permitted` | The host runs util-linux 2.41 (Debian 13), whose `nsenter --target` joins the namespace through a pidfd — a call the unit's sandbox rejects. Fixed in nkt 1.8.83, which names the namespace file instead (`--mount=/proc/1/ns/mnt`): update the host's binary. Installing `dbus` also sidesteps it, since the terminal then takes the `systemd-run` path and never reaches `nsenter` at all |
| Logging in with a system account does nothing | System login (`NKT_SYSTEM_LOGIN`, on by default in local mode) needs `unix_chkpwd` on the host and the account must be in `sudo`/`wheel`/`admin` — root is always allowed. It is deliberately unavailable in hub mode, where accounts are the hub's own |
