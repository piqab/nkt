...
список контейнеров и сетей через Docker Engine API, управление контейнерами,
и главное — цикл правки конфига с настоящим `nginx -t`, включая ветку отказа:
битый конфиг отклоняется, файл автоматически возвращается на место.

**Чего стенд не покрывает:**

* systemd — внутри контейнера его нет, источник `services` честно сообщает о
  недоступности, а действия start/stop/restart над юнитами не проверить;
