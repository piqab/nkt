...
* **резервный канал связи на случай недоступности SSH** — отдельная
  галочка на хосте: тот держит независимое от SSH постоянное соединение к
  хабу (WebSocket + TLS), так что дашборд и терминал продолжают работать,
  даже если входящий 22-й порт на хосте заблокируют файрволом. Хаб всегда
  сначала пробует обычный SSH, переключается на этот канал только при
  неудаче дозвона — не подменяет SSH для установки/обновления. Требует
  `NKT_HUB_PUBLIC_ADDR` на самом хабе — см. HUB.md, раздел 5;
