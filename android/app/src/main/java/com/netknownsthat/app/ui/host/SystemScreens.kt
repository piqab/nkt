    ...
    if (open != null || error != null || viewModel.openFileLoading) {
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            when {
                ...
                open != null -> Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                    Text(
                        text = open.path,
                        style = MaterialTheme.typography.titleSmall,
                        modifier = Modifier.padding(horizontal = 16.dp),
                    )
                    // Read-only: editing a config from a phone is a good way
                    // to take a service down by autocorrect. The web UI is
                    // the place for that; this shows what is actually there.
                    Text(
                        text = open.content,
