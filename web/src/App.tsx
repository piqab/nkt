      ...
      {collapsed ? (
        ...
        <>
          <div className="row" style={{ marginBottom: '0.4rem' }}>
            ...
            {/* На хабе приватный режим живёт в «О системе»; у одиночного
                nkt такой страницы нет — галочка здесь. */}
            {!isHub && (
              <Tooltip title={`${t('app.privacy')}: ${t('app.privacyHint')}`} placement="right">
                <input type="checkbox" aria-label={t('app.privacy')} checked={privacy} onChange={(e) => setPrivacy(e.target.checked)} style={{ margin: 0 }} />
              </Tooltip>
            )}
          </div>
          <div className="row" style={{ marginBottom: '0.4rem' }}>
