...
/**
 * Runs `apt-get install -y ufw` live, the same PTY/WebSocket bridge
 * UpdateModal uses for package upgrades — modeled on it directly, just
 * without the "confirm what apt is about to do" framing an upgrade needs:
 * installing one known package from the distro's own repos has no judgment
 * call attached to it the way "which services restart after this upgrade"
 * does, so -y is safe and there's nothing to prompt the operator for.
 */
export default function UfwInstallModal({
  onClose,
  onFinished,
  outcome,
  rescanning,
  ...
  return (
    ...
    >
      <p className="small muted">
        Выполняется <code className="mono">apt-get install -y ufw</code>.
      </p>
      ...
      {status === 'closed' &&
        (outcome === null || outcome === undefined ? (
          ...
          <Banner kind="info">
            ufw установлен. {rescanning ? 'Пересканирую хост…' : 'Хост пересканирован.'}
          </Banner>
        ) : (
          <Banner kind="error">
